...
{
  ros::init(argc, argv, "robots_task_generation");
  ros::NodeHandle n;
  float frequency = 200.0f;
  
  // std::string fileName;
  // RobotsTaskGeneration::SurfaceType surfaceType;  
  ...
  //   {
  //     surfaceType = RobotsTaskGeneration::SurfaceType::PLANAR;
  //   }
  //   else if(std::string(argv[2]) == "-s" && std::string(argv[3]) == "n")
  //   {
  //     surfaceType = RobotsTaskGeneration::SurfaceType::NON_FLAT;
  //   }
  //   else
  //   {
  ...
  RobotsTaskGeneration robotsTaskGeneration(n,frequency);

  if (!robotsTaskGeneration.init()) 
  {
    return -1;
  }
  else
