  ...
  _mode(mode)
{
	...
	ROS_INFO_STREAM("The move to desired pose node is created at: " << _n.getNamespace() << " with freq: " << frequency << "Hz.");
}
...
bool MoveToDesiredPose::init() 
{
  ...
  _stop = false;
  _getTransformed = false;
  _toolOffset = 0.0f;


  _subRealPose[RIGHT] = _n.subscribe<geometry_msgs::Pose>("/right_lwr/ee_pose", 1,boost::bind(&MoveToDesiredPose::updateRealPose,this,_1,RIGHT),ros::VoidPtr(),ros::TransportHints().reliable().tcpNoDelay());
  _pubDesiredTwist[RIGHT] = _n.advertise<geometry_msgs::Twist>("/right_lwr/joint_controllers/passive_ds_command_vel", 1);
  _pubDesiredOrientation[RIGHT] = _n.advertise<geometry_msgs::Quaternion>("/right_lwr/joint_controllers/passive_ds_command_orient", 1);


  _subRealPose[LEFT] = _n.subscribe<geometry_msgs::Pose>("/left_lwr/ee_pose", 1, boost::bind(&MoveToDesiredPose::updateRealPose,this,_1,LEFT),ros::VoidPtr(),ros::TransportHints().reliable().tcpNoDelay());
  _pubDesiredTwist[LEFT] = _n.advertise<geometry_msgs::Twist>("/left_lwr/joint_controllers/passive_ds_command_vel", 1);
  _pubDesiredOrientation[LEFT] = _n.advertise<geometry_msgs::Quaternion>("/left_lwr/joint_controllers/passive_ds_command_orient", 1);
  ...
  while (!_stop) 
  {
    if(_firstRealPoseReceived[LEFT] && _firstRealPoseReceived[RIGHT])
    {
      ...
      {       
        try
        {
          _lr.lookupTransform("/world", "/left_robot_basis",ros::Time::now(), _transform);
          _getTransformed = true;
        } 
...
void MoveToDesiredPose::publishData()
{
  ...
  // std::cerr << (int) _mode << std::endl;
  if(_mode == SINGLE_RIGHT || _mode == BOTH)
  {
  ...
  if(_mode == SINGLE_LEFT || _mode == BOTH)
  {
