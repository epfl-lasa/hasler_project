  ...
  case ALL_TOOLS:
    ...
    for (unsigned int i=0; i<NB_TOOLS; i++)
    {
      ...
      _subUnbiasedJointTorques[i] = _n.subscribe<custom_msgs::FootOutputMsg_v3>(
                  ...
                  ros::VoidPtr(), ros::TransportHints().reliable().tcpNoDelay());  
      _subToolJointStates[i] = _n.subscribe<sensor_msgs::JointState>( "/"+std::string(Tools_Names[i])+"_tool/joint_states"
...
void targetObject::readTFTrocar(unsigned int n_) {
  ...
  if (n_ == ALL_TOOLS) {
    ...
    destination_frame = std::string(Tools_Names[n_]) + "_tool_base_link";
    original_frame = "torso_link";
