...
public:
  enum tool_Types{ENDOSCOPE_ROBOT, GRASPER_ROBOT, GRASPER_HAND_R, GRASPER_HAND_L, NB_TOOLS};
  static const int NB_ROBOT_TOOLS =  NB_TOOLS-2; 
  enum tool_States {INSERTION_STATE, INSIDE_TROCAR_STATE};
  enum grasperA_State {A_POSITIONING_OPEN, A_GRASPING, A_HOLDING_GRASP, A_POSITIONING_CLOSE, A_FETCHING_OLD_GRASP, A_RELEASE_GRASP, NB_ACTIONS_GRASPER, NO_SHARED_CONTROL};
...
private:
  ...
  grasperA_State _grasperRobotSharedControl;
...
private:
  ...
  //Eigen
    ...
    Array2i _toolTextCoord[NB_ROBOT_TOOLS];
    
    Affine3d _allToolsPose[NB_TOOLS]; // Endoscope + Ex. Other 3 graspers
