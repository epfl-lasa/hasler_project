...
for file in $(find $CATKIN_WS/build -name compile_commands.json); do
    ...
    run-clang-tidy -header-filter="$CATKIN_WS/src/rokubimini_sdk/.*" -export-fixes $fixes_file -p $(dirname $file)
    i=$((i + 1))
done

linting_passed=true
for file in $(ls $CATKIN_WS/src/rokubimini_sdk/fixes-*.yaml); do
    if [ -s $file ]; then
        if (cat $file | grep "rokubimini_sdk"); then
