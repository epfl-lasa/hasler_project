...
{
bool createChildElement(TiXmlHandle& childHandle, const TiXmlHandle& parentHandle, std::string childName,
                        bool forceIfExisting)
{
  ...
  do
  {
    ...
    const bool exists =
        !(forceIfExisting && pos == std::string::npos) && child.FirstChild(subName).ToElement() != nullptr;
    child = exists ? child.FirstChild(subName).ToElement() :
                     child.ToElement()->LinkEndChild(new TiXmlElement(subName))->ToElement();
