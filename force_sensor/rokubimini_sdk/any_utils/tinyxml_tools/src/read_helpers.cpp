  ...
  while ((pos = childName.find('/')) != std::string::npos)
  {
    if (!tinyxml_tools::getChildHandle(parentHandle, parentHandle, childName.substr(0, pos), verbose))
    {
      if (verbose)
      {
        ...
        {
          MELO_WARN_STREAM("[getChildHandle] Could not get handle " << parentElement->ValueStr()
                                                                    << "::" << childName.substr(0, pos)
...
bool getChildElements(std::vector<TiXmlElement*>& childElements, const TiXmlHandle& parentHandle,
