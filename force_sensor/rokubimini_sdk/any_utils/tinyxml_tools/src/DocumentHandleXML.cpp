...
{
DocumentHandleXML::DocumentHandleXML()
  : DocumentHandle(), document_(), rootHandle_(&document_), seekedHandle_(&document_)
{
}
...
bool DocumentHandleXML::create(const std::string& path, DocumentMode mode, const std::string& rootName)
{
  ...
  if (readable())
  {
    ...
    {
      MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not create file " << path << ", has multiple roots.");
      return false;
    }
  }
  ...
  {
    document_.Clear();
    MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not create file " << path << ", mode is NONE.");
    return false;
  }
...
bool DocumentHandleXML::merge(const std::string& path, MergeMode mode, bool force)
{
  ...
  mergeDocumentHandle.create(path, DocumentMode::READ);
  auto mergeDocument = mergeDocumentHandle.getDocument();

  // Check if current document allows writing
  if (!force && !writeable())
  {
    MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not merge file " << path << ". Document is read-only.");
    return false;
  }
  ...
  {
    MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not merge file " << path
                                                                             << ". Documents have different Roots.");
    return false;
  }
...
bool DocumentHandleXML::seek(std::string handle)
{
  ...
  while (handle.substr(0, 3) == std::string("../"))
  {
    ...
    {
      MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] No parent handle when seeking "
                       << handle << ". Reset seeked handle to root handle.");
      seekedHandle_ = rootHandle_;
      return false;
    }
    // Set parent
  ...
  if (!handle.empty())
  {
    ...
    {
      MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Seeked handle "
                       << handle << " does not exist. Reset seeked handle to root handle.");
      seekedHandle_ = rootHandle_;
      return false;
    }
  }
...
bool DocumentHandleXML::resolveDefault()
{
  ...
  {
    MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not load default file " << defaultPath
                                                                                    << ". Could not load file.");
    return false;
  }

  // Replace merge
  mergeTrees(defaultDocumentHandle.rootHandle_, rootHandle_, MergeMode::REPLACE_OVERWRITE);
  rootHandle_.ToElement()->Clear();
  for (auto* child = defaultDocumentHandle.rootHandle_.FirstChild().ToElement(); child != nullptr;
...
bool DocumentHandleXML::resolveIncludes()
{
  ...
  for (auto& handle : handles)
  {
    if (handle.ToElement() != nullptr && handle.ToElement()->Parent()->ToElement() != nullptr)
    {
      ...
      {
        MELO_WARN_STREAM(
            "[tinyxml_tools::DocumentHandle]: Could not include file, no attribute dir in tag include_xml.");
        continue;
      }
      ...
      {
        MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Could not include file " << includePath
                                                                                   << ". Could not load file.");
        return false;
      }
...
{
  // Recurse
  for (TiXmlElement* childElement = tree.ToElement(); childElement != nullptr;
       childElement = childElement->NextSiblingElement())
  {
...
bool DocumentHandleXML::mergeTrees(TiXmlHandle tree1, TiXmlHandle tree2, MergeMode mode)
{
       ...
       tree2ChildElement = tree2ChildElement->NextSiblingElement())
  {
    ...
    if (tree1ChildElement == nullptr)
    {
      ...
      {
        MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Structure mismatch element " << tree2ChildElement->Value()
                                                                                       << " does not exist!");
        return false;
      }
      else
    ...
    {
      // Set attributes
      for (TiXmlAttribute* attribute = tree2ChildElement->FirstAttribute(); attribute != nullptr;
           attribute = attribute->Next())
      {
        ...
        if (tree1ChildElement->Attribute(attribute->Name()) == nullptr)
        {
          if (mode == MergeMode::REPLACE_OVERWRITE)
          {
            MELO_WARN_STREAM("[tinyxml_tools::DocumentHandle] Structure mismatch attribute " << attribute->Name()
                                                                                             << " does not exist!");
            return false;
          }
          else
