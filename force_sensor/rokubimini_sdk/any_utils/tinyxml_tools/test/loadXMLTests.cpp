...
TEST_F(LoadXMLFixture, loadPrimitiveTypes)
{  // NOLINT
  // Get handle
...
TEST_F(LoadXMLFixture, loadSTLContainers)
{  // NOLINT
  // Get handle
...
TEST_F(LoadXMLFixture, loadEigen)
{  // NOLINT
  // Get eigen vector
  ...
  ASSERT_TRUE(tinyxml_tools::loadParameter("Root/Eigen/Matrices/Dynamic", matrix22Dynamic, xmlDocumentHandle_,
                                           { "v15", "v30", "v45", "v60" }));
  ASSERT_TRUE(matrix22Dynamic.isApprox(matrix22DynamicDes));
}

TEST_F(LoadXMLFixture, loadSiblings)
{  // NOLINT
  // Get handle
  ...
  ASSERT_TRUE(
      tinyxml_tools::loadParameterSiblings("Map/kv", stringIntMap, siblingsHandle, std::make_pair("key", "value")));
