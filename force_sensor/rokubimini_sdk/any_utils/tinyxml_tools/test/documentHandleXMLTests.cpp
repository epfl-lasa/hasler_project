...
struct DocumentHandleXMLFixture : public ::testing::Test
{
  static void SetUpTestCase()
  {
    ...
    ASSERT_FALSE(
        xmlDocumentHandleMultipleRoots_->create("xml_files/MultipleRoots.xml", tinyxml_tools::DocumentMode::APPEND));
  }
...
TEST_F(DocumentHandleXMLFixture, mergeParams)
{  // NOLINT
  ASSERT_TRUE(xmlDocumentHandleMerge1_->merge("xml_files/Params.xml", tinyxml_tools::MergeMode::REPLACE_OVERWRITE));
}

TEST_F(DocumentHandleXMLFixture, checkMergeParams1)
{  // NOLINT
  testMerge(xmlDocumentHandleMerge1_);
}

TEST_F(DocumentHandleXMLFixture, checkMergeParams2)
{  // NOLINT
  testMerge(xmlDocumentHandleMerge2_);
}

TEST_F(DocumentHandleXMLFixture, checkMergeParams3)
{  // NOLINT
