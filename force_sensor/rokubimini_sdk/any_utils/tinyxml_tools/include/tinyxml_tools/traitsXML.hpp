...
struct elementXML : public element<T>
{
  ...
  template <typename T_ = type>
  static void
  write(const type& value, TiXmlElement* const element, const stringType& name,
...
struct elementXML<std::array<T, N>> : public element<std::array<T, N>>
{
  ...
  static bool readSiblings(type& value, TiXmlElement* element, const siblingStringType& name, const siblingType& def)
  {
    ...
    for (int i = 0; i < value.size(); ++i)
    {
      ...
      {
        MELO_WARN_STREAM("[tinyxml_tools]: Not enough elements with attribute name '"
                         << name << "' to load std::array of size " << N << ".")
        return false;
      }
...
template <typename T>
struct elementXML<T, typename std::enable_if<std::is_base_of<typename Eigen::MatrixBase<T>, T>::value>::type>
    : public element<T>
{
  // Forward types
  using Base = element<T>;
  ...
  static bool read(type& value, const TiXmlElement* element, const stringType& name, const type& def)
  {
    ...
    for (int i = 0; i < value.rows(); ++i)
    {
      for (int j = 0; j < value.cols(); ++j)
      {
        ...
        success = traits::elementXML<typename T::Scalar>::read(value(i, j), element, nameDef,
                                                               (def.size() ? def(i, j) : typename T::Scalar{})) &&
                  success;
      }
    }
    return success;
