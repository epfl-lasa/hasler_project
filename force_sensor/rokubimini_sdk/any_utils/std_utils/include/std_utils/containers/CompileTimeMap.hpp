...
public:
  ...
  static std::vector<KeyType_> findVector(ValueType_ value)
  {
    ...
    CompileTimeMap<KeyType_, ValueType_, KeyValuePair<KeyType_, ValueType_, Key_, Value_>,
                   REST_...>::findVectorRecursive(value, vec);
    return vec;
  }
  ...
  */
                                     ...
                                     std::function<bool(const KeyType_&, const ValueType_&)> f)
  {
    return containsRange(begin, end) &&
           (Key_ == begin ?
                f(Key_, Value_) &&
                    ((begin == end) ? true : CompileTimeMap<KeyType_, ValueType_, REST_...>::forEachRangePostBegin(
                                                 begin, end, f)) :
  ...
  */
  static constexpr std::size_t size()
  {
    return 1 + CompileTimeMap<KeyType_, ValueType_, REST_...>::size();
  }
                   ...
                   CompileTimeMap<ValueType2_, ValueType_, KVValues2_...>>
{
  using type = std_utils::ctm_concatenate_t<
      KeyType_, ValueType_, ctm_transform_t<KeyType_, ValueType_, CompileTimeMap<KeyType1_, ValueType1_, KV1_...>,
                   ...
                   KeyValuePair<InvertedValueType_, InvertedKeyType_, InvertedValue_, InvertedKey_>, Kv_...>>
{
  static_assert(CompileTimeMap<InvertedValueType_, InvertedKeyType_,
                               KeyValuePair<InvertedValueType_, InvertedKeyType_, InvertedValue_, InvertedKey_>,
                               Kv_...>::allValuesUnique(),
                "[ctm_invert] Could not invert non-unique map!");
  using type =
      ctm_insert_front_t<InvertedKeyType_, InvertedValueType_,
                         typename ctm_invert<InvertedKeyType_, InvertedValueType_,
                                             CompileTimeMap<InvertedValueType_, InvertedKeyType_, Kv_...>>::type,
