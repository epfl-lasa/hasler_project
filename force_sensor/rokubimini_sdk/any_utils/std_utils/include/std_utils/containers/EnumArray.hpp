...
public:
  ...
  explicit EnumArray(Item a) noexcept : EnumArray()
  {
    std::fill(this->begin(), this->end(), a);
  }

  //! Provide at, [] implementations for enums
  Item& operator[](Enum e) noexcept
  {
    return this->operator[](static_cast<std::size_t>(e));
  }
  constexpr const Item& operator[](Enum e) const noexcept
  {
    return this->operator[](static_cast<std::size_t>(e));
  }
  Item& at(Enum e)
  {
    return this->at(static_cast<std::size_t>(e));
  }
  constexpr const Item& at(Enum e) const
  {
    return this->at(static_cast<std::size_t>(e));
  }
