...
protected:
  ...
  struct Character
  {
    ...
    Character(Type value, std::string valueAsString)
      : value_(std::move(value)), valueAsString_(std::move(valueAsString))
    {
      yamlNode_ = value_;
    }
  };
  ...
  const std::vector<Character> characters_ = {
    ...
    Character("|", "\"|\""), Character("/", "/"),     Character(",", "\",\""), Character(".", "."),
    Character(";", ";"),     Character(":", "\":\""), Character("!", "\"!\""), Character("?", "\"?\""),
    Character("_", "_"),     Character("~", "~"),     Character("@", "\"@\""), Character("#", "\"#\""),
    Character("$", "$"),     Character("%", "\"%\""), Character("^", "^"),     Character("&", "\"&\""),
    Character("*", "\"*\""), Character("°", "°"),     Character("^", "^"),     Character(" ", "\" \"")
  };
};

TEST_F(TestYamlNodeScalarStringSpecialCharacters, throwsExceptionOnBadCast)
{  // NOLINT
...
TEST_F(TestYamlNodeScalarStringSpecialCharacters, accessValue)
{  // NOLINT
  for (const auto& character : characters_)
  {
...
TEST_F(TestYamlNodeScalarStringSpecialCharacters, writeToString)
{  // NOLINT
  for (const auto& character : characters_)
  {
...
TEST_F(TestYamlNodeScalarStringSpecialCharacters, readFromString)
{  // NOLINT
  for (const auto& character : characters_)
  {
