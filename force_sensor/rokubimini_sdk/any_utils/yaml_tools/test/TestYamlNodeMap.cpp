...
TEST_F(TestYamlNodeMap, readFromString)
{  // NOLINT
  EXPECT_EQ(yamlNode_, yaml_tools::YamlNode::fromString(yamlString_));
}

TEST_F(TestYamlNodeMap, writeToString)
{  // NOLINT
  EXPECT_EQ(yamlStringAlphabetical_, yamlNode_.toString());
}

TEST_F(TestYamlNodeMap, typeIsSetOnConstruction)
{  // NOLINT
...
TEST_F(TestYamlNodeMap, getSize)
{  // NOLINT
  EXPECT_EQ(4u, yamlNode_.size());
}

TEST_F(TestYamlNodeMap, nonConstDoesNotThrowExceptionOnGoodKey)
{                                        // NOLINT
  EXPECT_NO_THROW(yamlNode_[goodKey_]);  // NOLINT
}

TEST_F(TestYamlNodeMap, constDoesNotThrowExceptionOnGoodKey)
{  // NOLINT
  const yaml_tools::YamlNode const_yaml_node = yamlNode_;
  EXPECT_NO_THROW(const_yaml_node[goodKey_]);  // NOLINT
}

TEST_F(TestYamlNodeMap, nonConstDoesNotThrowExceptionOnBadKey)
{                                       // NOLINT
  EXPECT_NO_THROW(yamlNode_[badKey_]);  // NOLINT
}

TEST_F(TestYamlNodeMap, constThrowsExceptionOnBadKey)
{  // NOLINT
  const yaml_tools::YamlNode const_yaml_node = yamlNode_;
  EXPECT_THROW(const_yaml_node[badKey_], yaml_tools::Exception);  // NOLINT
}

TEST_F(TestYamlNodeMap, nonConstThrowsExceptionOnId)
{                                                          // NOLINT
  EXPECT_THROW(yamlNode_[anyId_], yaml_tools::Exception);  // NOLINT
}

TEST_F(TestYamlNodeMap, constThrowsExceptionOnId)
{  // NOLINT
  const yaml_tools::YamlNode const_yaml_node = yamlNode_;
  EXPECT_THROW(const_yaml_node[anyId_], yaml_tools::Exception);  // NOLINT
}

TEST_F(TestYamlNodeMap, memberAccessAndCasting)
{  // NOLINT
...
TEST_F(TestYamlNodeMap, iteratorBasedForLoop)
{  // NOLINT
  unsigned int i = 0;
...
TEST_F(TestYamlNodeMap, constIteratorBasedForLoop)
{  // NOLINT
  const yaml_tools::YamlNode yaml_node = yamlNode_;
  unsigned int i = 0;
  for (auto element = yaml_node.begin(); element != yaml_node.end(); element++)
  {
    const std::string key = element->first;
    const yaml_tools::YamlNode& value = element->second;
    // Elements must be alphabetic.
...
TEST_F(TestYamlNodeMap, rangeBasedForLoop)
{  // NOLINT
  unsigned int i = 0;
...
TEST_F(TestYamlNodeMap, constRangeBasedForLoop)
{  // NOLINT
  const yaml_tools::YamlNode yaml_node = yamlNode_;
  unsigned int i = 0;
  for (auto element : yaml_node)
  {
    const std::string key = element.first;
    const yaml_tools::YamlNode& value = element.second;
    // Elements must be alphabetic.
