...
TEST_F(TestYamlNodeScalarStringEmpty, throwsExceptionOnBadCast)
{  // NOLINT
...
TEST_F(TestYamlNodeScalarStringEmpty, accessValue)
{  // NOLINT
  EXPECT_EQ(value_, yamlNode_.as<std::string>());
}

TEST_F(TestYamlNodeScalarStringEmpty, writeToString)
{  // NOLINT
  EXPECT_EQ(valueAsString_, yamlNode_.toString());
}

TEST_F(TestYamlNodeScalarStringEmpty, readFromString)
{  // NOLINT
