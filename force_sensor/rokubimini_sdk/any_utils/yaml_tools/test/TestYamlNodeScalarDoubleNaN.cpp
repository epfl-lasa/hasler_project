...
TEST_F(TestYamlNodeScalarDoubleNaN, throwsExceptionOnBadCast)
{  // NOLINT
...
TEST_F(TestYamlNodeScalarDoubleNaN, accessValue)
{  // NOLINT
...
TEST_F(TestYamlNodeScalarDoubleNaN, writeToString)
{  // NOLINT
  EXPECT_EQ(valueAsString_, yamlNode_.toString());
}

TEST_F(TestYamlNodeScalarDoubleNaN, readFromString)
{  // NOLINT
  // Note: EXPECT_EQ cannot be used, as NaN == Nan returns false.
  EXPECT_TRUE(std::isnan(value_));
