...
{
  const std::string nested_key = nestedKey_ + "/" + key;
  try
  {
  ...
  {
    throw Exception(std::string("Checking for key '" + nested_key + "' failed: ") + std::string(exception.what()));
  }
}

bool YamlNode::hasKey(const size_t i) const
{
  const std::string nested_key = nestedKey_ + "/" + std::to_string(i);
  try
  {
...
std::vector<std::string> YamlNode::getKeys() const
{
  ...
  {
    throw Exception(std::string("Getting keys failed (are your keys of type string?): ") +
...
YamlNode YamlNode::operator[](const std::string& key)
{
  ...
  try
  {
    return YamlNode(impl_[key], nested_key);
  }
  catch (const YAML::Exception& exception)
  {
    throw Exception(std::string("Accessing node at '" + nested_key + "' failed: ") + std::string(exception.what()));
  }
}
...
YamlNode YamlNode::operator[](const std::string& key) const
{
  ...
  if (!hasKey(key))
  {
    throw Exception(std::string("Accessing node at '" + nested_key + "' failed: The key does not exist."));
  }
  try
  {
    return YamlNode(impl_[key], nested_key);
...
YamlNode YamlNode::operator[](const size_t i)
{
  ...
  try
  {
    return YamlNode(impl_[i], nested_key);
  }
  catch (const YAML::Exception& exception)
  {
    throw Exception(std::string("Accessing node at '" + nested_key + "' failed: ") + std::string(exception.what()));
