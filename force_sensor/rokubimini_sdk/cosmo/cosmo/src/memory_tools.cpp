...
T* find_check(boost::interprocess::managed_shared_memory& managedShm, const std::string& name)
{
  ...
  {
    MELO_FATAL_STREAM("Unable to find variable "
                      << name << " in shared memory. Shared memory seems corrupted, you should clean/remove it!");
  }
...
bool getMemoryInfo(const std::string& poolName, MemoryInfo* info)
{
  ...
  try
  {
...

    {
      ...
      boost::interprocess::sharable_lock<boost::interprocess::interprocess_sharable_mutex> poolLock(*(poolMutex),
                                                                                                    abs_time);
      ...
      if (poolLock.owns())
      {
        ...
        for (const auto& topic : *(topicList))
        {
          ...
          auto smMutex =
              find_check<boost::interprocess::interprocess_sharable_mutex>(managedShm, (topic + "__Mutex").c_str());
          ...
          //            const auto smNodeInfos = managedShm.find<Communicator::ShmNodeInfoVector>((topic +
          //            "__NodeInfos").c_str());
