...
public:
   ...
   *                                     set to true
   */
  SubscriberOptions(const std::string& topic, const SubscriberCallback& callback, bool autoSubscribe,
...
public:
  ...
  //! The arrived message is passed as const reference to the callback function. Note that the reference points to
  //! shared memory,
  ...
  //! If set to true, the last message published to the topic will be received by the subscriber, even if it was sent
  //! before subscribing.
  bool latch_ = false;
