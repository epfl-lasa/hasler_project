 ...
 * instruction (128bits)
 */
template <typename MessageType_, unsigned int Alignment_ = DEFAULT_ALIGNMENT>
...
public:
  ...
  Publisher() = delete;
  Publisher(const std::string& topic) : Publisher(std::make_shared<Options>(topic))
  {
  }

  Publisher(const PublisherOptionsPtr& options) : BaseType(options), options_(options)
  {
  }

  Publisher(Publisher&&) = default;  // NOLINT(performance-noexcept-move-constructor)
   ...
   */
  virtual bool publish(const MessageType_& message,
                       const std::chrono::microseconds& maxLockTime = std::chrono::microseconds{ 50 })
  {
    bool send_successful = false;
    {
      const boost::posix_time::ptime abs_time =
          boost::get_system_time() + boost::posix_time::microseconds(maxLockTime.count());
      boost::interprocess::scoped_lock<boost::interprocess::interprocess_sharable_mutex> lock(
          *(this->smMutex_), boost::interprocess::defer_lock);
      if (lock.timed_lock(abs_time))
      {
        if (*this->smNumSubscribers_ > 0u)
        {
          ...
          *this->smMessage_ = message;
          ...
          MELO_INFO("\e[1;36m<== Sent message over shm: Idx: %ld, Topic: %s \e[0m", *(this->smCounter_),
                    options_->topic_.c_str());
#endif
