...
/**
 *
 * @tparam     MessageType_  Message that will be stored in shared memory. Don not use dynamic sized memory and
 * std::string!
 * @tparam     Alignment_    Alignment of the memory allocation, has to be a multiple of size of a SIMD memory read
 * instruction (128bits)
 */
template <typename MessageType_, bool isPublisher_, unsigned int Alignment_ = DEFAULT_ALIGNMENT>
class Communicator
{
public:
  Communicator() = delete;
   ...
   */
  Communicator(const std::string& topic) : Communicator(std::make_shared<CommunicatorOptions>(topic))
  {
  }
   ...
   */
  Communicator(const std::shared_ptr<CommunicatorOptions>& options) : options_(options)
  {
    allocatePool();
    allocateTopic();
  }

  Communicator(Communicator&&) = default;  // NOLINT(performance-noexcept-move-constructor)

  /*!
   * Decrements smNumSubscribers_ or smNumPublishers_ respectively and deallocates the shared memory if the counters are
   * 0
   */
...
private:
  void allocatePool()
  {
    managedShm_ = { boost::interprocess::open_or_create, options_->memoryPoolName_.c_str(), 65536, nullptr,
  ...
  void allocateTopic()
  {
    ...
    boost::interprocess::scoped_lock<boost::interprocess::interprocess_sharable_mutex> pool_lock{ *smPoolMutex_ };

    const NodeInfoAllocator alloc_inst(managedShm_.get_segment_manager());
    try
    {
      smMutex_ = managedShm_.find_or_construct<boost::interprocess::interprocess_sharable_mutex>(
          (options_->topic_ + "__Mutex").c_str())();
      smCnd_ = managedShm_.find_or_construct<boost::interprocess::interprocess_condition_any>(
          (options_->topic_ + "__Cnd").c_str())();
      smHandle_ = managedShm_.find_or_construct<MemoryManager::handle_t>((options_->topic_ + "__Handle").c_str())();
      smNumPublishers_ =
          managedShm_.find_or_construct<unsigned int>((options_->topic_ + "__NumSubscribers").c_str())(0u);
      smNumSubscribers_ =
          managedShm_.find_or_construct<unsigned int>((options_->topic_ + "__NumPublishers").c_str())(0u);
      smCounter_ = managedShm_.find_or_construct<unsigned long int>((options_->topic_ + "__Counter").c_str())(0lu);
      smNodeInfos_ =
          managedShm_.find_or_construct<ShmNodeInfoVector>((options_->topic_ + "__NodeInfos").c_str())(alloc_inst);
    }
    ...
    if (lock.owns())
    {
      // check if there are existing subscribers or publishers which already allocated the memory for the MessageType_
      // instance
      ...
      else
      {
        ...
        try
        {
          void* message_ptr = managedShm_.allocate_aligned(sizeof(MessageType_), Alignment_);
          smMessage_ = new (message_ptr) MessageType_;
          *smHandle_ = managedShm_.get_handle_from_address(message_ptr);
        }
        catch (boost::interprocess::bad_alloc& ex)
        {
      ...
      for (auto iter = smNodeInfos_->begin(); iter != smNodeInfos_->end();)
      {
        if (!checkProcessExistence(*iter))
        {
          ...
          MELO_WARN_STREAM("Detected missing process on topic "
                           << options_->topic_ << " (pid: " << iter->pid_ << " / start time: " << iter->startTime_
    ...
    {
      --(*smNumNodes_);
      MELO_FATAL_STREAM("Failed to acquire topic '"
                        << options_->topic_ << "' mutex lock during initialization within "
                        << (double(options_->missingProcessTimeout_.count()) / 1e6)
                        << "s. Is there a crashed process blocking the mutex? Please clean shared memory.");
    }
  }
  ...
  void deallocateTopic()
  {
    ...
    const auto pid = getpid();
    smNodeInfos_->erase(
        std::find_if(smNodeInfos_->begin(), smNodeInfos_->end(), [pid](const ShmNodeInfoVector::value_type& info) {
    ...
    if (*smNumPublishers_ + *smNumSubscribers_ == 0)
    {
      ...
      else
      {
        //                it->~ShmString();
        const CharAllocator char_alloc(managedShm_.get_segment_manager());
        *it = ShmString(char_alloc);  // workaround for boost bug. Assigning an empty string makes use of the
                                      // boost::basic_string_base optimization for short strings,
        // which does not use heap memory and therefore not creates a memory leak for the boost vector bug
        // https://svn.boost.org/trac10/ticket/13500
  ...
  static unsigned int getProcessTime(const int pid)
  {
    ...
    proc_file.open("/proc/" + std::to_string(pid) + "/stat");

    if (proc_file.is_open())
    {
      char str[255];
      proc_file.getline(str, 255);  // delim defaults to '\n'
...
protected:
  // user should not do modifications on the options struct because e.g. changing topic or memory pool name will lead to
  // memory leaks.
