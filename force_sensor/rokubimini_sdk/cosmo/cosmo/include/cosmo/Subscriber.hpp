...
public:
  ...
  virtual bool receive(const std::chrono::microseconds& maxLockTime = std::chrono::microseconds{ 200 })
  {
    ...
    smLock_.lock();
    bool has_new_message = this->smCnd_->timed_wait(
        smLock_, abs_time, [=]() { return (*(this->smCounter_) > localCounter_ || !this->active_); });
    if (!this->active_)
    {
      unlockMutex();
    ...
    if (has_new_message)
    {
      ...
      MELO_INFO("\e[1;32m==> Received message over shm: Idx: %ld, Topic: %s \e[0m", *(this->smCounter_),
