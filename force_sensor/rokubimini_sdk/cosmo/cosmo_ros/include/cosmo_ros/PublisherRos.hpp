...
template <typename Msg_, typename MsgRos_, template <typename, typename> class Converter_ = ConversionTraits,
          unsigned int Alignment_ = DEFAULT_ALIGNMENT>
...
public:
   ...
   */
  PublisherRos(const std::string& topic) : PublisherRos(std::make_shared<PublisherRosOptions>(topic))
  {
  }
  ...
  PublisherRos(const PublisherRosOptionsPtr& options)
    ...
    , options_(options)
    , rosPublisher_(
          options->nodeHandle_.advertise<MsgRos_>(options_->topic_, options_->rosQueueSize_, options_->rosLatch_))
  ...
  PublisherRos(PublisherRos&&) = default;  // NOLINT(performance-noexcept-move-constructor)
  ...
  bool publish(const Msg_& message, const std::chrono::microseconds& maxLockTime = std::chrono::microseconds{ 1000 },
               typename std::enable_if<HasConvert<T, U, C>::VALUE>::type* /*unused*/ = nullptr)
  {
    // Write message to shared memory.
    bool send_successful = BaseType::publish(message, maxLockTime);

    // Stage the message.
    if (getNumRosSubscribers() > 0u || options_->rosLatch_)
    {
      send_successful &= stageMessage(Converter_<Msg_, MsgRos_>::convert(message), maxLockTime);
    }
    else
    {
   ...
   */
  template <typename T = Msg_, typename U = MsgRos_, template <typename, typename> class C = Converter_>
  bool publishAndSend(const Msg_& message,
                      const std::chrono::microseconds& maxLockTime = std::chrono::microseconds{ 1000 },
                      typename std::enable_if<HasConvert<T, U, C>::value>::type* /*unused*/ = 0)
  {
    bool send_successful = BaseType::publish(message, maxLockTime);
    if (rosPublisher_.getNumSubscribers() > 0u || options_->rosLatch_)
    {
  ...
  /*!
   *  Send all messages in the queue over ROS. If the publisher is not configured with autoPublishRos_ = true, this
