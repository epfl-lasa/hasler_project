...
inline std::string getNamespacedTopicName(ros::NodeHandle& nh, const std::string& paramName,
                                          const std::string& defaultTopic)
{
  std::string topic_name = param_io::param<std::string>(nh, paramName, defaultTopic);
  addNamespaceToTopicName(nh, topic_name);
  return topic_name;
}
                                                                  ...
                                                                  const std::string& defaultTopic)
{
  return advertiseShmRos<Msg_, MsgRos_, Converter_>(
      name, std::make_shared<PublisherRosOptions>(
                param_io::param<std::string>(nh, "publishers/" + name + "/topic", defaultTopic), nh));
}
...
{
  options->topic_ =
      param_io::param<std::string>(options->nodeHandle_, "publishers/" + name + "/topic", options->topic_);
  options->rosQueueSize_ =
      param_io::param<int>(options->nodeHandle_, "publishers/" + name + "/queue_size", options->rosQueueSize_);
...
inline SubscriberPtr<Msg_> subscribeShm(ros::NodeHandle& nh, const std::string& name, const std::string& defaultTopic,
                                        void (Obj_::*fp)(const Msg_&), Obj_* obj)
{
  auto options =
      std::make_shared<SubscriberOptions>(getNamespacedTopicName(nh, "subscribers/" + name + "/topic", defaultTopic),
...
{
  auto options =
      std::make_shared<SubscriberRosOptions<Msg_>>(defaultTopic, std::bind(fp, obj, std::placeholders::_1), nh);
...
{
  options->topic_ =
      param_io::param<std::string>(options->nodeHandle_, "subscribers/" + name + "/topic", options->topic_);
  options->rosQueueSize_ =
      param_io::param<int>(options->nodeHandle_, "subscribers/" + name + "/queue_size", options->rosQueueSize_);
