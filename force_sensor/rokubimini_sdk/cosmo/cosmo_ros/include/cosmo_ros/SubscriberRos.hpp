...
public:
  ...
  SubscriberRos(SubscriberRos&&) noexcept = default;
  ...
  bool receive(const std::chrono::microseconds& maxLockTime = std::chrono::microseconds{ 200 }) override
  {
    if (this->getNumShmPublishers() == 0)
    {
      ...
      receivedRosMsg_ = false;
      rosCallbackQueue_.callAvailable(
          ros::WallDuration(maxLockTime.count() / 1000000, (maxLockTime.count() % 1000000) * 1000));
...
protected:
  inline void subscribeRos()
  {
    ...
    ros::SubscribeOptions ops = ros::SubscribeOptions::create<MsgRos_>(
        ...
        boost::bind(&SubscriberRos<Msg_, MsgRos_, Converter_, Alignment_>::rosCallback, this, _1), ros::VoidPtr(),
        &rosCallbackQueue_);
