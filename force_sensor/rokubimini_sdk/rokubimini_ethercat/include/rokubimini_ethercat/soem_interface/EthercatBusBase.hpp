...
#include <rokubimini_ethercat/soem_interface/common/Macros.hpp>
...
public:
                    ...
                    const Value value)
  {
    ...
    if (wkc <= 0)
    {
      MELO_ERROR_STREAM("Slave " << slave << ": Working counter too low (" << wkc << ") for writing SDO (ID: 0x"
                   ...
                   Value& value)
  {
    ...
    if (wkc <= 0)
    {
      MELO_ERROR_STREAM("Slave " << slave << ": Working counter too low (" << wkc << ") for reading SDO (ID: 0x"
                                 << std::setfill('0') << std::setw(4) << std::hex << index << ", SID 0x"
                                 << std::setfill('0') << std::setw(2) << std::hex << static_cast<uint16_t>(subindex)
                                 << ").");
      return false;
