...
template <>
class ConversionTraits<any_measurements::ImuWithCovariance::CovarianceMatrix,
                       sensor_msgs::Imu::_linear_acceleration_covariance_type>
{
public:
...
public:
  ...
  inline static void convert(const Msg& msg, MsgRos& msgRos)
  {
    ...
    msgRos.position.resize(msg_size);
    msgRos.velocity.resize(msg_size);
    msgRos.effort.resize(msg_size);
...
public:
  ...
  inline static void convert(const Msg& msg, MsgRos& msgRos)
  {
    ...
    msgRos.position.resize(msg_size);
    msgRos.velocity.resize(msg_size);
    msgRos.acceleration.resize(msg_size);
    msgRos.effort.resize(msg_size);
    unsigned int i = 0;
  ...
  inline static void convert(const MsgRos& msgRos, Msg& msg)
  {
    ...
    {
      std::cout << "Trying to convert extended joint state message with length " << msgRos.position.size()
                << ", should be " << msg.size() << "!" << std::endl;
      return;
    }
...
public:
  ...
  inline static void convert(const MsgRos& msgRos, Msg& msg)
  {
    ...
    msg.positionCovarianceType_ =
        static_cast<any_measurements::NavSat::CovarianceType>(msgRos.position_covariance_type);
...
template <>
class ConversionTraits<any_measurements::PoseWithCovariance::CovarianceMatrix,
