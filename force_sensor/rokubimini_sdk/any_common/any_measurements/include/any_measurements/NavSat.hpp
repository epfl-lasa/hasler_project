...
public:
  ...
  NavSat(const Time& time, const Status status, const Service service, const double latitude, const double longitude,
         const double altitude, const CovarianceType positionCovarianceType, const CovarianceMatrix& positionCovariance)
    : time_(time)
    , status_(status)
    , service_(service)
    , latitude_(latitude)
