...
public:
  ...
  {
    double sec_d;
    uint64_t nsec = nanoSeconds() + static_cast<uint64_t>(std::modf(seconds, &sec_d) * 1e9);
    uint64_t sec = this->seconds() + static_cast<uint64_t>(sec_d);
    if (nsec > static_cast<uint64_t>(1e9))
    {
      sec++;
      nsec -= static_cast<uint64_t>(1e9);
  ...
  bool operator<(const Time& other) const
  {
    return ((time_.tv_sec < other.time_.tv_sec) ||
            (time_.tv_sec == other.time_.tv_sec && time_.tv_nsec < other.time_.tv_nsec));
  }
  ...
  {
    return std::chrono::steady_clock::time_point(std::chrono::seconds(time_.tv_sec) +
                                                 std::chrono::nanoseconds(time_.tv_nsec));
  }
  ...
  {
    return (time_.tv_sec > static_cast<time_t>(1e9));  // if the time was taken using the monotonic clock, the computer
                                                       // would need to run since several thousand days to get to this
                                                       // value...
  }
  ...
  {
    return time_.tv_sec == 0 && time_.tv_nsec == 0;
  }
