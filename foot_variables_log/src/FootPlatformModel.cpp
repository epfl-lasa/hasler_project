...
const float epsilon =1.224646799147353e-16; 
...
Eigen::Matrix4f FootPlatformModel::forwardKinematics(Eigen::Matrix<float,5,1> joints, bool anglesInDegree)
{
  ...
  float t6 = sin(phi);
  float t7 = sin(theta);
  float t9 = t6*t7*epsilon;
  float t8 = t5-t9;
  float t10 = t5*epsilon;
  float t11 = t6*t7;
  float t12 = t10+t11;
  H(0,0) = t2*t4*epsilon-t3*t8;
  H(1,0) = t2*t4+t3*t12;
  H(2,0) = -t4*t7+t2*t3*t6;
  H(3,0) = 0.0;
  H(0,1) = t2*t3*(-epsilon)-t4*t8;
  H(1,1) = -t2*t3+t4*t12;
  H(2,1) = t3*t7+t2*t4*t6;
  H(3,1) = 0.0;
  H(0,2) = t6+t5*t7*epsilon;
  H(1,2) = t6*(-epsilon)+t5*t7;
  H(2,2) = t2*t5;
  H(3,2) = 0.0;
