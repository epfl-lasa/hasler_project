...
bool footVarLogger::init() //! Initialization of the node. Its datatype (bool) reflect the success in initialization
{
	...
	if (_platform_name==LEFT){
		...
		_subFootOutput = _n.subscribe<custom_msgs::FootOutputMsg>(PLATFORM_PUBLISHER_NAME_LEFT, 1, boost::bind(&footVarLogger::fetchFootOutput, this, _1), ros::VoidPtr(), ros::TransportHints().reliable().tcpNoDelay());
		...
		_pubMotorsEffortM = _n.advertise<custom_msgs::FootOutputMsg>("FI_Data/Left/motors_invDyn", 1);
	...
	if (_platform_name==RIGHT){
		...
		_subFootOutput = _n.subscribe<custom_msgs::FootOutputMsg>(PLATFORM_PUBLISHER_NAME_RIGHT, 1, boost::bind(&footVarLogger::fetchFootOutput, this, _1), ros::VoidPtr(), ros::TransportHints().reliable().tcpNoDelay());
		...
		_pubMotorsEffortM = _n.advertise<custom_msgs::FootOutputMsg>("FI_Data/Right/motors_effort", 1);
...
void footVarLogger::footDataTransformation()
{
  ...
  void footVarLogger::fetchFootOutput(const custom_msgs::FootOutputMsg::ConstPtr &msg)
  {
