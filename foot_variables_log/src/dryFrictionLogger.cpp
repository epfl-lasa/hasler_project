...
_rawFilename(filename_)
{
	...
	for (int k = 0; k < NB_AXIS; k++)
	{
		if (k == _nAxis)
		{
			_desiredMotorsPosition(_nAxis) = C_WS_LIMITS[_nAxis] * des_positions_[0];
		}
		else
		{
			_desiredMotorsPosition(k) = C_WS_LIMITS[k] * des_positions_[(int)(rand() % (NB_POSITIONS+1))];
		}
	}
...
bool dryFrictionLogger::init() //! Initialization of the node. Its datatype (bool) reflect the success in initialization
{
	...
	if (_platform_name==LEFT){
		_subFootOutput = _n.subscribe<custom_msgs::FootOutputMsg>(PLATFORM_PUBLISHER_NAME_LEFT, 1, boost::bind(&dryFrictionLogger::fetchFootOutput, this, _1), ros::VoidPtr(), ros::TransportHints().reliable().tcpNoDelay());
		_pubFriction = _n.advertise<custom_msgs::FootOutputMsg>("FI_Data/Left/friction_data", 1);
	...
	if (_platform_name==RIGHT){
		_subFootOutput = _n.subscribe<custom_msgs::FootOutputMsg>(PLATFORM_PUBLISHER_NAME_RIGHT, 1, boost::bind(&dryFrictionLogger::fetchFootOutput, this, _1), ros::VoidPtr(), ros::TransportHints().reliable().tcpNoDelay());
		_pubFriction = _n.advertise<custom_msgs::FootOutputMsg>("FI_Data/Right/friction_data", 1);
...
void dryFrictionLogger::generateNewRefPoints()
{
		for (int k = 0; k < NB_AXIS; k++)
		{
			if (k == _nAxis)
			{
				_desiredMotorsPosition(_nAxis) = C_WS_LIMITS[_nAxis] * des_positions_[_nPositions];
			}
			else
			{
				_desiredMotorsPosition(k) = C_WS_LIMITS[k] * des_positions_[int(rand() % NB_POSITIONS)];
			}
		}
}
...
void dryFrictionLogger::fetchFootOutput(
	const custom_msgs::FootOutputMsg::ConstPtr &msg)
{
