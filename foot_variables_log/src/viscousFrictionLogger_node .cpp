...
{
  viscousFrictionLogger::Platform_Name platform_id_;
  ...
  if (argc>=2)
  {
    if (std::string(argv[1])=="left") {platform_id_=viscousFrictionLogger::LEFT;}
    else if (std::string(argv[1])=="right") {platform_id_=viscousFrictionLogger::RIGHT;}
    else
    {
  ...
  viscousFrictionLogger viscousFrictionLogger(nh_,frequency,platform_id_, axis_, filename_);  

  if (!viscousFrictionLogger.init()) 
  {
    return -1;
  }
  else
  {
    viscousFrictionLogger.run();
  }
  return 0;
}
